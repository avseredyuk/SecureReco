...
public class FileSelector {
	...
	private void prepareFilterSpinner(String[] filesFilter) {
		...
		if (filesFilter == null || filesFilter.length == 0) {
			filesFilter = new String[] { IOUtil.FILTER_ALLOW_ALL };
			mFilterSpinner.setEnabled(false);
		}
		ArrayAdapter<String> adapter = new ArrayAdapter<>(mContext, R.layout.spinner_item, filesFilter);
	...
	private void makeList(final File location, final String filesFilter) {
		...
		if (listFiles != null) {
			...
			for (int index = 0; index < listFiles.length; index++) {
				...
				if (IOUtil.accept(tempFile, filesFilter)) {
	...
	private void setSaveLoadButton(final FileOperation operation) {
		Button mSaveLoadButton = (Button) mDialog.findViewById(R.id.fileSaveLoad);
		switch (operation) {
			case SAVE:
	...
	/** Simple wrapper around the Dialog.dismiss() method. */
	public void dismiss() {
		mDialog.dismiss();
	}
}
